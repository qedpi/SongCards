...
/* QEDPI's Latte */(function(){var Transposer,draw_polygon,indexOf=[].indexOf||function(item){for(var i=0,l=this.length;i<l;i++){if(i in this&&this[i]===item)return i;}return -1;};Transposer=__webpack_require__(1);draw_polygon=function draw_polygon(){var poly;poly=$('#polygon').getContext('2d');poly.fillStyle='#f00';poly.beginPath();poly.moveTo(0,0);poly.lineTo(200,50);poly.lineTo(50,200);poly.lineTo(0,90);poly.closePath();poly.fill();return 0;};window.onload=function(){return draw_polygon();};$(function(){var animate_body,color_formatter,into_lines,major_to_both,major_to_minor,music_format,music_formatter,music_unformat,original,original_key,original_text,pretty,scroll_speed,scroll_speed_fast,scroll_speed_medium,scroll_speed_slow,scroll_state_off,scroll_state_on,semitone_offset,set_key_to,shareClipboard,share_link,stop_animation,temp,transpose_by,transpose_to,uke_domain,words;semitone_offset=0;major_to_minor={"Eb":"C","E":"C#","F":"D","Gb":"Eb","G":"E","Ab":"F","A":"F#","Bb":"G","B":"G#","C":"A","Db":"Bb","D":"B"};uke_domain='http://www.ukulele-tabs.com/images/ukulele-chords/';into_lines=function into_lines(text){return text.split('\n');};words=function words(line){return line.split(' ');};major_to_both=function major_to_both(key){return key+'/'+major_to_minor[key]+'m';};set_key_to=function set_key_to(result){$('#lyrics').html(result.text);return $('#new-key').val(major_to_both(result.key));};music_format=function music_format(text){return text.replace(/#/g,'♯').replace(/b/g,'♭');};music_unformat=function music_unformat(text){return text.replace(/♯/g,'#').replace(/♭/g,'b');};music_formatter=function music_formatter(sym,id){return sym.replace('#','♯').replace('b','♭');};color_formatter=function color_formatter(sym,id){var loc,str;str=music_format(sym);if(indexOf.call(sym,'m')>=0){loc=str.indexOf('m');str=str.slice(0,loc).toLowerCase()+str.slice(loc+1);}str=str.replace(/M/g,'');sym=sym.replace(/M/g,'');sym=sym.replace(/\//g,'-');str=str.replace(/(\d+)/g,'<sup>$1</sup>');return '<b><a href="'+uke_domain+sym+'.png"<span class="xxlarge c'+(id%7+1)+'">'+str+'</span></a></b>';};transpose_by=function transpose_by(steps){var lyrics_text;lyrics_text=original_text;return set_key_to(Transposer.transpose(lyrics_text).withFormatter(color_formatter).up(steps));};transpose_to=function transpose_to(key){var lyrics_text;lyrics_text=original_text;return set_key_to(Transposer.transpose(lyrics_text).withFormatter(color_formatter).toKey(key));};original=Transposer.transpose($('#lyrics').text()).up(0);original_key=major_to_both(original.key);original_text=original.text;transpose_by(0);$('#transpose-up').click(function(){semitone_offset=(semitone_offset+1)%12;return transpose_by(semitone_offset);});$('#transpose-down').click(function(){semitone_offset=(semitone_offset-1)%12;return transpose_by(semitone_offset);});$('#transpose-reset').click(function(){return transpose_to(original_key.split('/')[0]);});$('#new-key').change(function(){return alert('hello');});shareClipboard=new Clipboard('#share_link_passcode');temp=$('#test-case').text();pretty={val:2}; /**
	    #my docstring
	     *
	     */ /*
